...
import { normalizeOKXMessage, buildOKXSubscription } from "./okx-normalizer.js";

export interface OKXMarketStreamOptions {
  url: string;
  symbols: string[];
...
export class OKXMarketStream {
  ...
  constructor(private readonly options: OKXMarketStreamOptions) {
  ...
  connect(): void {
    ...
    this.socket.on("open", () => {
      ...
      const subscription = buildOKXSubscription(this.options.symbols);
    ...
    this.socket.on("message", (raw) => {
      try {
        ...
        const tick = normalizeOKXMessage(message);
